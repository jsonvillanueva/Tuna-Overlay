...
async function updateAlbumArtSmooth(newSrc) {
  try {
    // If file path is missing, fallback immediately
    if (!newSrc || newSrc.trim() === "") {
      albumArtEl.src = PLACEHOLDER;
      blurBgEl.style.backgroundImage = `url('${PLACEHOLDER}')`;
      state.lastAlbumArt = "";
      return;
    }

    // Only refresh if cover OR metadata changed
    if (
      newSrc === state.lastAlbumArt &&
      state.lastTitle === titleEl.textContent &&
      state.lastArtist === artistEl.textContent
    ) {
      return;
    }

    // Delay is IMPORTANT (Tuna writes file slightly late)
    setTimeout(async () => {
      ...
      // Apply image
      albumArtEl.src = base64;
      blurBgEl.style.backgroundImage = `url('${base64}')`;

      state.lastAlbumArt = newSrc;

      console.log("[AlbumArt] Updated");
    }, 700);
  } catch (err) {
    console.error("[AlbumArt] Failed, using placeholder", err);
    albumArtEl.src = PLACEHOLDER;
    blurBgEl.style.backgroundImage = `url('${PLACEHOLDER}')`;
  }
}
